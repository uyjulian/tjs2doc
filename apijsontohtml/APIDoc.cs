...
namespace apijsontohtml {
	...
	class ScriptNode {
		...
		public void WriteMain( string title, string about ) {
			...
			writer.WriteLine( "<frame src=\"frame_index.html\" name=\"index\" id=\"index\" title=\"index\" />" );
			writer.WriteLine( "<frame src=\"about.html\" name=\"main\" id=\"main\" title=\"Contents\" />" );
			writer.WriteLine( "<noframes><body>Please see with a browser that supports frames</body></noframes>" );
		...
		private void WriteIndex() {
			...
			WriteHtmlHeader( ref writer, "table of contents" );
			writer.WriteLine( "<body>" );
			writer.WriteLine( "<h1><a id=\"id126\" name=\"id126\">Class reference</a></h1><div class=\"para\">" );

			List<ScriptNode> globalFunc = new List<ScriptNode>();
			Members.Sort( ( a, b ) => String.Compare( a.Name, b.Name ) );
			foreach( ScriptNode n in Members ) {
				if( n.Type == NodeType.Class ) {
					writer.WriteLine( "<a target=\"main\" class=\"jump\" href=\"class_" + n.Name + ".html\">" + n.Name + " class</a><br />" );
					n.Members.Sort( ( a, b ) => String.Compare( a.Name, b.Name ) );
					foreach( ScriptNode cn in n.Members ) {
						if( cn.Type == NodeType.Class ) {
							writer.WriteLine( "<a target=\"main\" class=\"jump\" href=\"class_" + n.Name + "." + cn.Name + ".html\">" + n.Name + "." + cn.Name + " class</a><br />" );
						}
					}
				}
			}
			...
			if( Members.FindIndex( s => s.Type == NodeType.Function ) >= 0 ) {
				writer.WriteLine( "<h1><a id=\"id127\" name=\"id127\">Function reference</a></h1><div class=\"para\">" );
				foreach( ScriptNode n in Members ) {
					if( n.Type == NodeType.Function ) {
						writer.WriteLine( "<a target=\"main\" class=\"jump\" href=\"func__" + n.Name + ".html\">" + n.Name + " function</a><br />" );
					}
				}
				writer.WriteLine( "</div>" );
			}
			if( Members.FindIndex( s => s.Type == NodeType.Property ) >= 0 ) {
				writer.WriteLine( "<h1><a id=\"id127\" name=\"id127\">Property reference</a></h1><div class=\"para\">" );
				foreach( ScriptNode n in Members ) {
					if( n.Type == NodeType.Property ) {
						writer.WriteLine( "<a target=\"main\" class=\"jump\" href=\"prop__" + n.Name + ".html\">" + n.Name + " Property</a><br />" );
					}
				}
				writer.WriteLine( "</div>" );
			}
			if( Members.FindIndex( s => s.Type == NodeType.Const ) >= 0 ) {
				writer.WriteLine( "<h1><a id=\"id127\" name=\"id127\">Constant reference</a></h1><div class=\"para\">" );
				foreach( ScriptNode n in Members ) {
		...
		public int WriteClassHtml( string owner ) {
			...
			writer.WriteLine( "<h1>Member</h1><div class=\"para\">" );
			writer.WriteLine( "<dl>" );
			if( constructorCount > 0 ) {
				writer.WriteLine( "<dt>Constructor</dt>" );
				writer.WriteLine( "<dd>" );
				var i = 0;
				foreach( ScriptNode n in Members ) {
					if( Name.Equals( n.Name ) ) {
			...
			if( Members.FindIndex( s => s.Type == NodeType.Function && Name.Equals( s.Name ) == false ) >= 0 ) {
				writer.WriteLine( "<dt>Method</dt>" );
				writer.WriteLine( "<dd>" );
				foreach( ScriptNode n in Members ) {
			...
			if( Members.FindIndex( s => s.Type == NodeType.Property ) >= 0 ) {
				writer.WriteLine( "<dt>Property</dt>" );
				writer.WriteLine( "<dd>" );
				foreach( ScriptNode n in Members ) {
					if( n.Type == NodeType.Property ) {
			...
			if( Members.FindIndex( s => s.Type == NodeType.Event ) >= 0 ) {
				writer.WriteLine( "<dt>Event</dt>" );
				writer.WriteLine( "<dd>" );
				foreach( ScriptNode n in Members ) {
			...
			if( Members.FindIndex( s => s.Type == NodeType.Const ) >= 0 ) {
				writer.WriteLine( "<dt>Constant</dt>" );
				writer.WriteLine( "<dd>" );
				foreach( ScriptNode n in Members ) {
					if( n.Type == NodeType.Const ) {
		...
		public void WritePropertyHtml( string owner ) {
			...
			writer.WriteLine( "<dl>" );
			writer.WriteLine( "<dt>Function/Meaning</dt>" );
			writer.WriteLine( "<dd>" + removeReturn(Comment.Summary) + "</dd>" );
			writer.WriteLine( "<dt>Type</dt>" );
			if( owner != null && owner.Length > 0 )
				writer.WriteLine( "<dd><a class=\"jump\" href=\"" + "class_" + owner + ".html\">" + owner + " Class</a> Properties</dd>" ); // Do you need a read-only perspective?
			else
				writer.WriteLine( "<dd>Global properties</dd>" );
			writer.WriteLine( "<dt>Explanation</dt>" );
			...
			if( Comment.See != null && Comment.See.Count > 0 ) {
				writer.WriteLine( "<dt>Reference</dt>" );
				writer.WriteLine( "<dd>" );
		...
		public void WriteFunctionHtml( ScriptNode parent, string owner, bool ev = false, int index = 0 ) {
			...
			if( ev ) {
				if( owner != null && owner.Length > 0 )
					writer.WriteLine( "<dd><a class=\"jump\" href=\"" + "class_" + owner + ".html\">" + owner + "class</a>Event</dd>" );
				else
					writer.WriteLine( "<dd>Global event</dd>" );
			} else {
				if( owner != null && owner.Length > 0 )
					writer.WriteLine( "<dd><a class=\"jump\" href=\"" + "class_" + owner + ".html\">" + owner + "class</a>method of</dd>" );
				else
					writer.WriteLine( "<dd>Global method</dd>" );
			}
			writer.WriteLine( "<dt>Syntax</dt>" );
			...
			writer.WriteLine( "<dt>Arguments</dt>" );
			writer.WriteLine( "<dd><table rules=\"all\" frame=\"box\" cellpadding=\"3\" summary=\"" + Name + "Arguments of\">" );
		...
		private void WriteHtmlHeader( ref StreamWriter writer, string title ) {
			...
			writer.WriteLine( "<link href=\"api.css\" type=\"text/css\" rel=\"stylesheet\" title=\"APIStandard style for reference\" />" );
